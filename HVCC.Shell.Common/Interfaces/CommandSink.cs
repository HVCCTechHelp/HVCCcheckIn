/*
 * The ComandSink implementation was taken from Josh Smith's 
...
namespace HVCC.Shell.Common
{
    ...
    {
        #region Data
        private readonly Dictionary<ICommand, CommandCallbacks> commandToCallbacksMap = new Dictionary<ICommand, CommandCallbacks>();
        #endregion // Data
        ...
        public void RegisterCommand(ICommand command, Predicate<object> canExecute, Action<object> execute)
        {
            ...
            this.commandToCallbacksMap[command] = new CommandCallbacks(canExecute, execute);
        }

        public void UnregisterAllCommands()
        {
            this.commandToCallbacksMap.Clear();
        }
        ...
        public void UnregisterCommand(ICommand command)
        {
            ...
            if (this.commandToCallbacksMap.ContainsKey(command))
            {
                this.commandToCallbacksMap.Remove(command);
            }
        }
        ...
        public virtual bool CanExecuteCommand(ICommand command, object parameter, out bool handled)
        {
            ...
            {
                handled = true;
                return this.commandToCallbacksMap[command].CanExecute(parameter);
            }
            else
            {
                handled = false;
        ...
        public virtual void ExecuteCommand(ICommand command, object parameter, out bool handled)
        {
            ...
            {
                handled = true;
                this.commandToCallbacksMap[command].Execute(parameter);
            }
            else
            {
                handled = false;
